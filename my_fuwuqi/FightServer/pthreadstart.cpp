...
void ChuLiAgemessClient(struct message * message,int * io)
{
	...
	//获取模块入口
	if (mess_ti.head().msgid() == CS_MSGID_Mate)
	{
	...
	{
		std::cout << "OnClientMsg Fail !" << std::endl;
		MYLOG.printflog("OnClientMsg Fail ！");     
		return;
	}
}
...
void ChuLiAgemessServer(struct message * message,int * io)
{
	...
	if (mess_ti.head().msgid() == SS_MSGID_FightServer)
	{
		IHandler* handler = HANDLERLIST->GetHandler();
		//执行模块功能
		handler->OnServerMsg(mess_ti,*io);		//游戏服务器主动向别人转发数据，然后别的服务器返回就调用OnServerMsg
	}
	else
	{
...
void * chulimessage(void *canshu)
{
	...
	while(1)
	{
		...
		while(!messageduilie.empty())   //如果队列有数据将要处理
		{
			...
			if(GAMECLIENT->GetSocketIo() == test.io)
